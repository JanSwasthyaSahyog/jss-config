...
where ((result.result_type='N' AND result.value != '' AND result.value IS NOT NULL AND (cast(result.value as double precision) NOT BETWEEN result.min_normal AND result.max_normal))
OR (result.result_type='D' and dictionary.local_abbrev = 'POSITIVE'))
AND date(referral.referral_request_date) between '#startDate#' and '#endDate#' and referral.canceled = FALSE
order by organization_name asc, sample.received_date desc, accession_number asc;
